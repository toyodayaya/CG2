...
int WINAPI WinMain(_In_ HINSTANCE, _In_opt_ HINSTANCE, _In_ LPSTR, _In_ int)
{
	...
	// Index用の頂点リソースを作る
	Microsoft::WRL::ComPtr <ID3D12Resource> indexResourceSprite = dxBasis->CreateBufferResources(sizeof(uint32_t) * 6);
	// 頂点バッファビューを作成する
	...
	// Sprite用の頂点リソースを作る
	Microsoft::WRL::ComPtr <ID3D12Resource> vertexResourceSprite = dxBasis->CreateBufferResources(sizeof(VertexData) * 6);
	...
	// マテリアル用のリソースを作る
	Microsoft::WRL::ComPtr <ID3D12Resource> materialResource = dxBasis->CreateBufferResources(sizeof(Material));
	// マテリアルにデータを書き込む
	...
	// WVP用のリソースを作る
	Microsoft::WRL::ComPtr <ID3D12Resource> wvpResource = dxBasis->CreateBufferResources(sizeof(TransformationMatrix));
	// データを書き込む
	...
	// Sprite用のリソースを作る
	Microsoft::WRL::ComPtr <ID3D12Resource> transformationMatrixResourceSprite = dxBasis->CreateBufferResources(sizeof(TransformationMatrix));
	// データを書き込む
	...
	while (true)
	{
		...
		// wvp用のCBufferの場所を設定
		dxBasis->GetCommandList()->SetGraphicsRootConstantBufferView(1, wvpResource->GetGPUVirtualAddress());
		...
		// IBVを設定
		dxBasis->GetCommandList()->IASetIndexBuffer(&indexBufferViewSprite);
		// SRVを設定
		dxBasis->GetCommandList()->SetGraphicsRootDescriptorTable(2, textureSrvHandleGPU);
		// SpriteCBufferの場所を設定
